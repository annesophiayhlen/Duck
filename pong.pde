class MyPaddle {
  ...
  void sideCollision() {    
    if (x > width - _width) { //paddle collision on sides
      x = width - _width;
    }  
    if (x < 0) {
      x = 0;
  ...
  /* //Collision/bounce off from paddle
   if (duck.newX + (duck.sizeE/2) > x && duck.newX < x + _width && duck.newY + (duck.sizeE/2) > y && duck.newY < y + _height) {
   duck.speed *= -1;
   score++;
   }
   if (dist(duck.newX + (duck.sizeE/2), duck.newY + (duck.sizeE/2), x, y) < x) { //Left side bounce of the paddel
...
/* 
 ...
 float heightR = 25;
 float xRSpeed = 5;
     ellipse(395,264,130,130);
     ellipse(newX+35*size, newY+22*size, sizeE, sizeE);
 //Collision/bounce off from rect( width/2, height-25, 100, 25)
